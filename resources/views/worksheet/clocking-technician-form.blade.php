...
@section('content')
    <div class="container p-5 h-100 moco-layout-height" style="min-width: 50vw">
        <div class="card">
            <div class="card-header text-center font-weight-bold">
                <h2 class="blue-grey-darker-hover">{{__('Technician clocking')}}</h2>
            </div>
            <div class="card-body">
                <div class="d-flex justify-content-between p-2">
                    ...
                    <div class="mr-2 mt-1 moco-color-error moco-row-table-font-large">{{__('Worksheet Number')}} :</div>
                    <input id="number" name="number" class="form-control form-control" style="width: auto" autocomplete="off" value=""/>
                        </div>
                        <div class="d-flex flex-row justify-content-end">
                    <div id="technician_field" class="mr-2 mt-1 moco-color-error moco-row-table-font-large">{{__('Technician number')}} :</div>
    ...
    <!-- Script -->
    <script type="text/javascript" src="{{ asset('js/moco.clock.js') }}"></script>
    <script type="text/javascript">
        $(function (){
            /*
            * fonction pour l'horloge
            */
            initClock('{{Auth::user()->language}}');
            $.ajaxSetup({
                headers: {
            ...
            var _url_worksheetcheck = '{{route('clocking.ajaxworksheetcheck')}}'
            var _url_teccheck = '{{route('clocking.ajaxtechniciancheck')}}'
            /*
            * init les champs
            */
            initField();
